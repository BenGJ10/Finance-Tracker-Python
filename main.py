...
class CSV:
    ...
    @classmethod    # access to class itself not to instances
    def initialize_csv(cls):
        try:
            pd.read_csv(cls.CSV_FILE)
        except FileNotFoundError:
            df = pd.DataFrame(columns = cls.columns)
            df.to_csv(cls.CSV_FILE, index = False)  # converting to csv file

    @classmethod    # dict containing data to add to CSV file
    def add_entry(cls, date, amount, category, description):
        ...
        # context manager (as) opens in append mode and closes automatically with no mem leaks
    ...
    def get_transactions(cls, start_date, end_date):
        ...
        # applying mask -> to rows, to see if we should select that row or not
        mask = (df["date"] >= start_date) & (df["date"] <= end_date)
        filter_df = df.loc[mask]    # filtered date frame locating all masked rows
        if filter_df.empty:
